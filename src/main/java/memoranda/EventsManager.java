...
/*			_root.addNamespaceDeclaration("jnevents", NS_JNEVENTS);
	...
	//Long Parameter List - 8 input parameters Refactored to accept a parameter object RepeatableEventInput
	public static IEvent createRepeatableEvent(RepeatableEventInput eventInput) {
		Element el = new Element("event");
		...
		el.addAttribute(new Attribute("repeat-type", String.valueOf(eventInput.getType())));
		el.addAttribute(new Attribute("id", Util.generateId()));
		el.addAttribute(new Attribute("hour", String.valueOf(eventInput.getHourVal())));
		el.addAttribute(new Attribute("min", String.valueOf(eventInput.getMinuteVal())));
		el.addAttribute(new Attribute("startDate", eventInput.getStartDate().toString()));
		if (eventInput.getEndDate() != null)
			el.addAttribute(new Attribute("endDate", eventInput.getEndDate().toString()));
		el.addAttribute(new Attribute("period", String.valueOf(eventInput.getPeriod())));
		// new attribute for wrkin days - ivanrise
		el.addAttribute(new Attribute("workingDays",String.valueOf(eventInput.isWorkDays())));
		el.appendChild(eventInput.getNoteText());
		rep.appendChild(el);
		return new EventImpl(el);
	}
			...
			if (date.inPeriod(ev.getStartDate(), ev.getEndDate())) {
				if (ev.getRepeat() == REPEAT_DAILY) {
					...
					if ((n - ns) % ev.getPeriod() == 0)
						v.add(ev);
				} else if (ev.getRepeat() == REPEAT_WEEKLY && date.getCalendar().get(Calendar.DAY_OF_WEEK) //SER 316 ASSIGN 7 Compressed If conditionals
						== ev.getPeriod()) {
						v.add(ev);
				} else if (ev.getRepeat() == REPEAT_MONTHLY && date.getCalendar().get(Calendar.DAY_OF_MONTH) //SER 316 ASSIGN 7 Compressed If Conditionals
						== ev.getPeriod()) {
						v.add(ev);
